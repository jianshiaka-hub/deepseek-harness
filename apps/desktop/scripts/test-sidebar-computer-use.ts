...
const page = createServer((request, response) => {
  ...
  if (request.url === '/cross-origin') {
    response.end(`<!doctype html><title>Cross-origin capture</title><h1>Cross-origin capture</h1><iframe id="foreign" src="${embeddedOrigin}/frame" onload="document.body.dataset.embeddedReady='yes'" style="width:260px;height:240px"></iframe>`)
    return
  }
