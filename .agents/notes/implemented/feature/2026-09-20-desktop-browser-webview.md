...
The Desktop toolbar has no sandbox-disable switch or remote-debugging endpoint. The later selected-tab Computer Use broker binds actions to an authenticated owner, exact guest and short lease rather than exposing all application targets; its [subframe unload replay](2026-09-26-sidebar-beforeunload-replay.md) also checks the original document before continuing. The separation of a target/lifetime handle from navigation commands follows the same distinction as Playwright's Android WebView and Page objects; device-level input is a separate responsibility.
